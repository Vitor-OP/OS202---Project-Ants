...
class Pheromon:
    ...
    def mark(self, the_position, has_WESN_exits,phero):
        assert(the_position[0] >= 0)
        assert(the_position[1] >= 0)
        cells = np.array([phero[the_position[0]+1, the_position[1]] if has_WESN_exits[d.DIR_WEST] else 0.,
                          phero[the_position[0]+1, the_position[1]+2] if has_WESN_exits[d.DIR_EAST] else 0.,
                          phero[the_position[0]+2, the_position[1]+1] if has_WESN_exits[d.DIR_SOUTH] else 0.,
                          phero[the_position[0], the_position[1]+1] if has_WESN_exits[d.DIR_NORTH] else 0.], dtype=np.double)
