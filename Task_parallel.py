...
if __name__ == "__main__":
    ...
    # if there are any remaining ants, we add them to the first process
    if rank == 1:
    ...
    total_iterations = 2000  # max iteration
    ...
    while iteration_count < total_iterations:
        ...
        if rank != 0:
            ...
            # data for visualization in core 0
            ...
            pheromones_process = [None for _ in range(size)]
            ...
            for i in range(1, size):
                ...
                age_process[i], historic_path_process[i], directions_process[i], food_counter_local, pheromone_local = received_data
                ...
                global_pheromones.append(pheromone_local)
            
            # update the pheromones
            global_pheromones = [update for update in global_pheromones if update is not None]
            global_pheromones = np.max(global_pheromones, axis=0)
            pherom.pheromon = global_pheromones
            
            # pass the data to the global colony
